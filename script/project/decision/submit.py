...
def state_text(text: str) -> str:
    if text in {"approve", "approved", "accept", "accepted"}:
        return "approved"
...
def role_decision(role: str, target_role: str) -> None:
    role_ok(role)
    role_ok(target_role)

    if ROLE[role] <= ROLE[target_role]:
        raise ValueError("role cannot decide target")


def decision_path(root: Path, slug: str, name: str) -> Path:
    return root / "project" / slug / "decision" / name / "meta.yaml"
...
def submit_decision(
    ...
    role_decision(role, target_role)
    project_ok(root, slug)

    if not text.strip():
        raise ValueError("empty text")

    path = decision_path(root, slug, name)
    ...
    return Path(
        write_yaml(
            str(path),
            decision_data(
                root, slug, name, role, target_role, target, decision, text, run
            ),
        )
    )
...
def main(argv: list[str]) -> None:
    if len(argv) < 9:
        raise ValueError(
            "usage: submit.py ROOT SLUG ID ROLE TARGET_ROLE TARGET DECISION TEXT"
        )
