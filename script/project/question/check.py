...
def check_item(root: Path, path: Path) -> list[dict[str, Any]]:
    ...
    if data.get("state") == "answered":
        for field in ["answer", "answer_role", "answer_run", "answered"]:
            if field not in data:
                bad.append(bad_item(path, "field", field))
...
def check_question(root: Path) -> list[dict[str, Any]]:
    base = root / "project"
    ...
    for path in sorted(base.glob("*/question/*/meta.yaml")):
        rel = path.relative_to(root)
        slug = rel.parts[1]
        name = rel.parts[3]
        ...
        if not slug_ok(name):
            bad.append(bad_item(path, "question", name))
...
def main(argv: list[str]) -> None:
    ...
    try:
        bad = check_question(root)

        for item in bad:
            run.logger.error(jline("question", "check", "bad", item))

        if bad:
            raise RuntimeError("question check failed")
